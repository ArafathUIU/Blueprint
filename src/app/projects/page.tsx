...
const STATUS_COLORS: Record<string, string> = {
  ...
  generating_stories: "bg-amber-600 text-white",
  generating_wireframes: "bg-purple-600 text-white",
  generating_prd: "bg-green-700 text-white",
  generating_roadmap: "bg-cyan-600 text-white",
};
  ...
  if (projects.length === 0) {
    return (
      <div className="relative z-10 mx-auto flex max-w-4xl flex-col items-center gap-4 px-6 py-24 text-center">
        <div className="mb-2 flex h-16 w-16 items-center justify-center rounded-2xl bg-zinc-900 border border-zinc-800">
        ...
        <Link href="/new" className="mt-3 inline-flex h-10 items-center justify-center rounded-lg bg-red-600 px-6 text-sm font-medium text-white hover:bg-red-500">
  ...
  return (
    <div className="relative z-10 mx-auto max-w-4xl px-6 py-12">
      <div className="mb-8 flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-white">Your Blueprints</h1>
          <p className="mt-1 text-sm text-zinc-500">{projects.length} project{projects.length !== 1 ? "s" : ""}</p>
        </div>
        <Link href="/new" className="inline-flex h-9 items-center justify-center rounded-lg bg-red-600 px-4 text-sm font-medium text-white hover:bg-red-500">
          New Blueprint
        </Link>
      </div>
